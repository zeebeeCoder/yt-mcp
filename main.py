...
def check_api_keys():
    ...
    if missing:
        ...
        if missing:
            ...
            console.print("- YOUTUBE_API_KEY=your_key_here")
            console.print("- OPENAI_API_KEY=your_key_here")
            console.print("- GOOGLE_GENAI_API_KEY=your_key_here")
            return False
...
def display_results(result, output_format: str = "rich", output_file: Optional[str] = None):
    ...
    if output_format == "json":
        ...
        if output_file:
            ...
            console.print(f"\n[green]Results also saved to: {output_file}[/green]")
...
@click.option('--format', 'output_format', type=click.Choice(['rich', 'json', 'markdown']), default='rich', help='Output format')
...
@click.option('--transcript-only', is_flag=True, help='Extract and process only transcript (shortcut)')
@click.option('--comments-only', is_flag=True, help='Extract and process only comments (shortcut)')
def analyze(video_url: str, instruction: str, max_comments: int, max_words: int,
           output: str, output_format: str, verbose: bool, log_file: str, 
           no_transcript: bool, no_comments: bool, no_transcript_processing: bool,
           no_comments_processing: bool, no_synthesis: bool, no_evaluation: bool,
           transcript_only: bool, comments_only: bool):
...
def setup():
    ...
    console.print("Setting up YouTube Analysis Pipeline...\n")

    # Check current API key status
    if not check_api_keys():
        # Create .env template
        Config.create_env_template()
        console.print("\n[green]Created .env.template file. Please copy it to .env and add your API keys.[/green]")
    else:
        console.print("\n[green]✓ All API keys are configured correctly![/green]")
    ...
    table.add_row("Max Total Words", str(config.max_total_word_length))
    table.add_row("OpenAI Model", config.openai_model)
    table.add_row("Gemini Model", config.gemini_model)
    table.add_row("Selected Questions", str(config.num_selected_questions))
